...
void Game::UpdateModel()
{
	...
	{
		count += 0.001f;
		for (int ct = 0; obstacles.size() < int(count) && ct < 40; ++ct)
			addObstacle();
		...
		if (wnd.kbd.KeyIsPressed(VK_UP))
			if (player.lowerY() == 320)
				player.goUp();
		if (wnd.kbd.KeyIsPressed(VK_DOWN))
			if (!player.inhibitDown)
				player.goDown(), player.inhibitDown = true;
			else;
		else
...
void Game::addObstacle()
{
	...
	do
	{
		isDone = true;
		x = value(random);
		Obstacle obstacle(x, 296);

		for (const auto& existingObstacle : obstacles)
			if (Entity::checkCollision(obstacle, existingObstacle, 75))
				isDone = false;

	} while (!isDone);

	obstacles.push_back(Obstacle(x, 296));
}
...
{
	for (auto i = 0; i < gfx.ScreenWidth; ++i)
		for (auto j = 0; j < gfx.ScreenHeight; ++j)
			gfx.PutPixel(i, j, 76, 211, 255);
